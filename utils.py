...
def plot_predictions(model, test_loader, device, classes: list, means, stds):
    ...
    images, labels, preds, probab = [], [], [], []
    for _ in range(20):
        img, lbl = dataiter.next()
        img, lbl = img.to(device), lbl.tolist()
        with torch.no_grad():
            # get sample outputs
            output = model(img)
        ...
        images.append(torch.squeeze(img[:, :, :1, :, :]))
        labels.extend(lbl)
        preds.append(pred.tolist())
        probab.extend(probs_tensor.tolist())
    ...
    for idx in np.arange(20):
        ...
        ax.set_title(
            "{} ({}) {:.2f}".format(
                classes[preds[idx]], classes[labels[idx]], probab[idx]
            ),
...
def calculate_means_std(files, root_dir):
    ...
    for idx in range(len(files)):
        for i in range(6):
            filenames.append(os.path.join(root_dir, files[idx]+f'_{i}.jpg'))
    ...
    print("*" * 80)
    print("means(rgb): ", means.tolist())
    print("stds(rgb): ", stds.tolist())
    print("*" * 80)
    return means, stds
