...
def convlstm(seq_len, img_height, img_width, channels):
    ...
    model.add(layers.ConvLSTM2D(filters = 64, kernel_size = (5, 5), return_sequences = True, 
    ...
    model.add(layers.BatchNormalization())
    model.add(layers.MaxPooling3D((1, 2, 2)))
    model.add(layers.ConvLSTM2D(filters = 64, kernel_size = (1, 1), return_sequences = True, 
                                activation='relu', padding='same'))
    model.add(layers.BatchNormalization())
    model.add(layers.MaxPooling3D((1, 2, 2)))
    model.add(layers.ConvLSTM2D(filters = 1, kernel_size = (3, 3), return_sequences = True, 
