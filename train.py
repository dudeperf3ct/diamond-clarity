...
from utils import calculate_means_std, get_train_transforms, get_val_transforms, set_global_seeds
from utils import colorstr, Params, datasets_to_df, set_logger
...
class ClassifierModel:
    ...
    def _prepare_training_generators(self, train_df: pd.DataFrame, root_dir: str, is_kfold: bool = False,
                                     fold: int = -1) -> tuple:
        """
        ...
        Args:
            ...
            root_dir (str) : Base path to images if path are relative in the dataframe
        ...
        # # run this once to get means and stds
        # means, stds = calculate_means_std(train_df['file'].values, self.root_dir)
        ...
        train_dataset = ImageClassifierDataset(
            img_paths=list(train_x), lbls=list(train_y), root_dir=root_dir, transform=trn_transforms
        )
        val_dataset = ImageClassifierDataset(
            img_paths=list(val_x), lbls=list(val_y), root_dir=root_dir, transform=val_transforms
        )
    ...
    def train(self, train_df: pd.DataFrame):
        ...
        cws = class_weight.compute_class_weight("balanced", classes=np.unique(train_y), y=train_y)
        logging.info(f"Class weights for labels: {cws}")

        logging.info("Loading data")
        train_loader, val_loader = self._prepare_training_generators(train_df, self.root_dir)
    ...
    def train_k_folds(self, train_df: pd.DataFrame):
        ...
        for f in range(self.folds):
            logging.info("Loading data")
            train_loader, val_loader = self._prepare_training_generators(train_df, self.root_dir, True, f)
            train_loader_length = len(train_loader)
            # Create training and validation dataloaders
            dataloaders_dict = {"train": train_loader, "val": val_loader}
